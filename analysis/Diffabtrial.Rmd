...
design <- model.matrix(~ 0 + Cond , phenoData)
...
# Fit model
dupcor <- duplicateCorrelation(normalizedData , design, block = phenoData$Ind)
fit <- lmFit(object = normalizedData, design = design, block = phenoData$Ind, correlation = dupcor$consensus)
fit2 <- eBayes(fit)
...
ggplot2::autoplot(prcomp_res, data = as.data.frame(phenoData), colour = "Cond", shape = "Ind", size =4)+
  theme_bw()
...
Model_counts <- DGEList(na.omit(Model_counts))
Model_counts$samples <- phenoData_RUVs
Model_counts$counts %>% dim()

# Fit model
dupcor <- duplicateCorrelation(Model_counts$counts, design, block = phenoData_RUVs$Ind)
fit <- lmFit(object = Model_counts$counts, design = design, block = phenoData_RUVs$Ind, correlation = dupcor$consensus)
fit2 <- eBayes(fit)
...
# Toptable summary organized to contain results for all tested proteins   
toptable_summary <- topTable(fit2, coef = "DoxvNorm",number = (nrow(Model_counts$counts)), p.value = 1, adjust.method = "none")
toptable_summary$Protein <- rownames(toptable_summary)
toptable_summary$P.Value %>% hist(breaks = 100)
...
# PCA of log2-quantile normalized-RUVs values:
Model_counts_PCA <- na.omit(Model_counts$counts)
sum(is.na(Model_counts))
prcomp_res <- prcomp(t(Model_counts_PCA), scale. = TRUE, center = TRUE)
